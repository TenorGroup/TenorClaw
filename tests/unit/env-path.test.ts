...
describe('env-path', () => {
  ...
  it('prepends entry with Windows delimiter', () => {
    setPlatform('win32');
    const next = prependPathEntry({ Path: 'C:\\Windows\\System32' }, 'D:\\tenorclaw\\resources\\bin');
    expect(next.path).toBe('D:\\tenorclaw\\resources\\bin;C:\\Windows\\System32');
    expect(next.env.Path).toBe('D:\\tenorclaw\\resources\\bin;C:\\Windows\\System32');
  });

  it('prepends entry with POSIX delimiter', () => {
    setPlatform('linux');
    const next = prependPathEntry({ PATH: '/usr/bin:/bin' }, '/opt/tenorclaw/bin');
    expect(next.path).toBe('/opt/tenorclaw/bin:/usr/bin:/bin');
    expect(next.env.PATH).toBe('/opt/tenorclaw/bin:/usr/bin:/bin');
  });
});
