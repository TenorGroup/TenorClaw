 ...
 * Used by TenorClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the TenorClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged TenorClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  TenorClaw sets
 ...
 * OpenClaw/TenorClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When TenorClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for TenorClaw to properly integrate with its updated tool system.
    ...
    // but TenorClaw historically stored them only under `channels.<type>.accounts.default`.
