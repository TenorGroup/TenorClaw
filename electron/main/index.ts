...
import { ensureTenorClawContext, repairTenorClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.tenorclaw.desktop';
...
// On Wayland this maps the running window to tenorclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('tenorclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[TenorClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[TenorClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[TenorClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== TenorClaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain TenorClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureTenorClawContext()
  // previously created the file before the gateway could seed the full template.
  void repairTenorClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensureTenorClawContext().catch((error) => {
        logger.warn('Failed to re-merge TenorClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge TenorClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureTenorClawContext will retry until the target files appear.
  void ensureTenorClawContext().catch((error) => {
    logger.warn('Failed to merge TenorClaw context into workspace:', error);
  });
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second TenorClaw instance detected; redirecting to the existing window');
